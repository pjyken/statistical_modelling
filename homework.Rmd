...
The above code performs the order selection test for additivity. It uses the null hypothesis $Y = \beta_0 + \beta_1 x_1 + \beta_2 x_2$. It then tests every nested model sequence up to an order of 3. It makes use of the `poly()` function, which converts the original longitude and latitude data into orthogonalized polynomials. 
